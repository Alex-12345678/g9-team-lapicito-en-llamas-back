    ...
    this.miFormulario
      ...
      ?.valueChanges.pipe(
        switchMap((value) => {
          ...
          return this.ecoService.getClasificaciones(value, value2, 'en');
        })
      )
      .subscribe((resp: any) => {
        ...
        this.clasficaciones = Object.keys(preferredLabel).map((key) => ({
          key,
  ...
  onDescription() {
    ...
    this.ecoService
      ...
      .subscribe((resp: any) => {
        ...
        // const { preferredLabel } = resp._embedded.results[0];
        // //tranformar de objeto a array
        // this.clasficaciones = Object.keys(preferredLabel)
        //   .map(key => ({ key, value: preferredLabel[key] }));
        // console.log([preferredLabel]);
      });
  }
}
